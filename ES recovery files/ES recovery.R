...
comment_times_stage2<-clean_combined_data2 %>%
  filter(str_detect(Comments, regex("Stun Stage 2", ignore_case = TRUE))) %>%
  group_by(Source_File) %>%
  slice_min(relative_time, n = 1) %>%
  ungroup() %>%
  select(Source_File, relative_time)

comment_times_stage2 <- comment_times_stage2 %>%
  rename(stage2_time = relative_time)

stage2_diff <- comment_times_stun %>%
  rename(stun_time = relative_time) %>%
  left_join(comment_times_stage2, by = "Source_File") %>%
